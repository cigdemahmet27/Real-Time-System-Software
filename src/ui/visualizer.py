...
def detect_input_policy(filename):
    """Reads the input.txt to find the requested server policy."""
    policy = "Background (None)"
    if not os.path.exists(filename): return "Unknown Input File"
    try:
    ...
    with open(filename, 'r') as f:
        ...
        for line in lines:
            ...
            if event in ["Running", "ServerExec", "BackgroundRun", "ServerExec(DS)"]:
                ...
                # --- FIX: Upgrade Description to Server if detected ---
                # If we previously thought this was just "Aperiodic", but now we see "Server", update it!
                if "Server" in desc and "Server" not in tasks[task_id]["desc"]:
                    tasks[task_id]["desc"] = desc

                current_intervals = tasks[task_id]["intervals"]
                if current_intervals and current_intervals[-1][0] + current_intervals[-1][1] == time:
                    start, dur = current_intervals.pop()
                    current_intervals.append((start, dur + 1))
                else:
                    current_intervals.append((time, 1))
...
def plot_gantt(tasks, misses, policy_name):
    ...
    # Sort: Put Server-related tasks at the top
    ...
    for i, task_id in enumerate(sorted_ids):
        ...
        # Color logic: Any description containing "Server" gets Orange
    ...
    ax.set_xlim(0, max(max_run, max_miss) + 2)
...
if __name__ == "__main__":
    policy_name = detect_input_policy(CONFIG_FILE)
    print(f"Detected Policy: {policy_name}")
    tasks, misses = parse_data(DATA_FILE)
    if tasks: plot_gantt(tasks, misses, policy_name)